...
const addFavorite = async (req, res) => {
  ...
  try {
    const newFavorite = await favoriteModel.createFavoriteModel(
      id_user,
      id_product
    );
    ...
    if (
      error.message.includes("Ya tienes este producto añadido a favoritos.")
    ) {
      // Devolvemos 400 Bad Request, con el mensaje específico.
...
const removeFavorite = async (req, res) => {
  ...
  try {
    const deletedFavorite = await favoriteModel.deleteFavoriteModel(
      id_user,
      id_product
    );
    res.status(200).json({
